...
namespace SASEBO_G_Checker
{
  ...
  public class Controller 
  {
	...
	//************************************************ Method
	//------------------------------------------------ Constructor
	public Controller (CipherModule.IBlockCipher module, int num_trace, string key_st, int Delay)
    {
      this.module    = module;
	...
	private bool mainloop_AES(CipherModule.IBlockCipher hw, CipherTool.IBlockCipher sw, ref int progress)
	{
     ...
     // myWriter.WriteLine(BitConverter.ToString(key).Replace("-", " "));
      ...
      { 
        ...
        // COMPARE SW & HW GENERATED CIPHERTEXTS
        //for (int i=0; i < 16; i++) 
          //  if (text_out[i] != text_ans[i]) 
            //    error = true;

        //WRITING TRACE NUMBER TO FILE
        //myWriter.Write(loop+",");

        // WRITING PLAINTEXT TO FILE
        //string tempString1 = BitConverter.ToString(text_in).Replace("-", " ");
        //myWriter.WriteLine(tempString + ",");

        // WRITING CIPHERTEXT TO FILE
        //string tempString2 = BitConverter.ToString(text_out).Replace("-", " ");
        //myWriter.WriteLine(loop + "," + tempString1 + "," + tempString2 );
        //myWriter.WriteLine();
