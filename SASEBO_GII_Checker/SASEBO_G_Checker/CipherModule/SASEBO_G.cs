...
namespace CipherModule
{
  ...
  public class SASEBO_G : IBlockCipher
  {
    ...
    public void writeDelay(byte[] text, int len)
    {
      writeBurst(ADDR_DELAY0, text, len);
    }

    public void readTrace(byte[] text, int len)
    {
     readBurst(ADDR_OTRAC0, text, len);
    }
